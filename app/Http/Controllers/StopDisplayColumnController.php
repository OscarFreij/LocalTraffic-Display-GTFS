...
class StopDisplayColumnController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        {
            $stop_times_earlyNEW = DB::table('stop_times')->where('stop_times.stop_id', '=', $id)->join('trips', 'trips.trip_id', '=', 'stop_times.trip_id')->join('calendar_dates', 'trips.service_id', '=', 'calendar_dates.service_id')->where('calendar_dates.date', '=', date('Ymd'))->where('departure_time', '>=', $timeP24)->orderBy('departure_time')->get()->toArray();
            $stop_times_lateNEW = DB::table('stop_times')->where('stop_times.stop_id', '=', $id)->join('trips', 'trips.trip_id', '=', 'stop_times.trip_id')->join('calendar_dates', 'trips.service_id', '=', 'calendar_dates.service_id')->where('calendar_dates.date', '=', date('Ymd'))->where('departure_time', '>=',$timeP0)->orderBy('departure_time')->get()->toArray();
        ...
        else if ($masterStop->location_type == 1)
        {
            ...
            $stop_times_earlyNEW = DB::table('stop_times')->whereIn('stop_times.stop_id', $ids)->join('trips', 'trips.trip_id', '=', 'stop_times.trip_id')->join('calendar_dates', 'trips.service_id', '=', 'calendar_dates.service_id')->where('calendar_dates.date', '=', date('Ymd'))->where('departure_time', '>=', $timeP24)->orderBy('departure_time')->get()->toArray();
            $stop_times_lateNEW = DB::table('stop_times')->whereIn('stop_times.stop_id',  $ids)->join('trips', 'trips.trip_id', '=', 'stop_times.trip_id')->join('calendar_dates', 'trips.service_id', '=', 'calendar_dates.service_id')->where('calendar_dates.date', '=', date('Ymd'))->where('departure_time', '>=',$timeP0)->orderBy('departure_time')->get()->toArray();
            $stop_times_array = $stop_times_earlyNEW + $stop_times_lateNEW;
        }
